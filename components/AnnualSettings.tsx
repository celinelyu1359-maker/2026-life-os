...
const AnnualSettings: React.FC = () => {
  ...
  // State for adding new dimension items inline
  ...
  const confirmAddDimensionItem = (dimId: string) => {
      if(newItemText.trim()) {
          setDimensions(prev => prev.map(d => 
            d.id === dimId ? {...d, items: [...d.items, {id: Date.now().toString(), text: newItemText, completed: false, actualResult: ''}]} : d
          ));
      }
  ...
  const removeDimensionItem = (dimId: string, itemId: string) => {
    setDimensions(prev => prev.map(d => 
        d.id === dimId ? {...d, items: d.items.filter(i => i.id !== itemId)} : d
      ));
  }
